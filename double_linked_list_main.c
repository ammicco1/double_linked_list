...
int main(int argc, char **argv){
    ...
    while(1){
        ...
        printf("\t- 11: GET PREV OF A NODE\n");
        printf("\t- 12: GET NODE INFORMATION\n");
        ...
        switch(choise){
            case 1: if(!init){
                        ...
                        list = inzialize_double_linked_list(inizialize_element(inzialize_key(key))); 
            ...
            case 11: if(init){
                        printf("INSERT THE INDEX: "); scanf("%d", &key); printf("THE NODE WITH INDEX %d HAS PREV: %d", key, get_element_key(get_prev_node(get_node_by_index(list, key)) -> elem));
                    }else{
                        printf("INIZIALIZE THE LIST FIRST!\n");
                    } break;
            case 12: if(init){
